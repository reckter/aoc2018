...
fun <Node> dijkstraInt(
    ...
    extraComparator: Comparator<List<Node>>? = null
): Pair<List<Node>, Int>? {
    return dijkstra(start, end, 0, Int::plus, getNeighbors, getWeightBetweenNodes, extraComparator)
}
...
fun <Node, Weight> dijkstra(
    ...
    val queue =
        PriorityQueue<Pair<List<Node>, Weight>>(Comparator.comparing<Pair<List<Node>, Weight>?, Weight> { it.second }
            .let {
                if (extraComparator != null) it.thenComparing(
                    { it.first },
                    extraComparator
                ) else it
            }
    ...
    while (queue.isNotEmpty()) {
        ...
        if(next.first.last() in seen) {
            continue
        }
        seen.add(next.first.last())
...
fun <Node, Weight> dijkstra(
    ...
    getWeightBetweenNodes: (from: Node, to: Node) -> Weight,
    extraComparator: Comparator<List<Node>>? = null
): Map<Node, Pair<List<Node>, Weight>> where Weight : Number, Weight : Comparable<Weight> {
    val queue =
        PriorityQueue<Pair<List<Node>, Weight>>(Comparator.comparing<Pair<List<Node>, Weight>?, Weight> { it.second }
            .let {
                if (extraComparator != null) it.thenComparing(
    ...
    val values = mutableMapOf<Node, Pair<List<Node>, Weight>>()
    queue.add(listOf(start) to zero)

    while (queue.isNotEmpty()) {
        val next = queue.remove()
        values[next.first.last()] = next
...
fun bisectInt(start: Int, end: Int, evaluate: (Int) -> Boolean): Int {
    var start = start
    var end = end

    while (start != end - 1) {
        val middle = start + (end - start) / 2
        println("curent range $start .. $end")
        val smaller = evaluate(middle)
        if (smaller) {
            end = middle
        } else
            start = middle
...
fun Int.toBigInt(): BigInteger = BigInteger.valueOf(this.toLong())
...
fun <T1, R> memoize(f: (T1) -> R) = Memoize1(f)

class Memoized<in T, out R>(val f: (T) -> R) {
    private val values = mutableMapOf<T, R>()
...
public fun <T, R> memoized(f: (T) -> R): Memoized<T, R> = Memoized(f)

class Memoize2<in T1, in T2, out R>(val f: (T1, T2) -> R) : (T1, T2) -> R {
    private val values = mutableMapOf<Pair<T1, T2>, R>()
    override fun invoke(t1: T1, t2: T2): R {
        return values.getOrPut(t1 to t2) { f(t1, t2) }
    }
}

fun <T1, T2, R> ((T1, T2) -> R).memoize(): (T1, T2) -> R = Memoize2(this)

fun <T1, T2, R> memoize(f: (T1, T2) -> R) = Memoize2(f)
